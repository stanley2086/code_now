...
# 本地没有设备用例的记录统计
def countSumNoDevices(devices, result, _read, phone_name,version,software_ID):

    if _read is None:
        _read = []
    # get_phone = getPhoneInfo(devices)
    # phone_name = get_phone["brand"] + "_" + get_phone["model"] + "_" + "android" + "_" + get_phone["release"]
    app = {"phone_name": phone_name, "pass": 0, "fail": 0, "device": devices,"version":version,'software_ID':software_ID}
    if result:
        app["pass"] = 1
    else:
        app["fail"] = 1
...
# 统计各个设备成功失败的用例数
def countSumDevices(devices, result, phone_name,version,software_ID):
    ...
    if _read:
        for item in _read:
            if item["device"] == devices:  # 本地已经存在该设备记录
                ...
                write(data=_read, path=PATH("../Log/" + Element.DEVICES_FILE))
                return
    countSumNoDevices(devices, result, _read, phone_name,version,software_ID)
    print(read(PATH("../Log/" + Element.DEVICES_FILE)))
