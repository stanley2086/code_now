...
def getPhoneInfo(devices):
    ...
    version = "ro.build.display.id=" #版本
    internal_version = "ckt.internal.version="#内部版本
    for line in phone_info:
         for i in line.split():
            ...
            if temp.find(release) >= 0:
                ...
                break
            if temp.find(internal_version) >= 0:
                result["internal_version"] = temp[len(internal_version):]
                break
